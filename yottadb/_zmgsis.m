...
multipart(%content,%nvhead,%payload,%boundary) ; parse content type: multipart/form-data
 ...
 . . s hname=$$trim($p(header,":",1)," "),hvalue=$$trim($p(header,":",2,9999)," ") i hname="" q
 . . i hname="Content-Disposition" s sname=$p($p($p(hvalue,"name=",2)," ",1),";",1) s:$e(sname)="""" @("sname="_sname)
...
gparse(key,ref) ; parse global reference
 ...
 k key s key=0
 s blk="",sort=1,type=1
 s x=$p($e(ref,1,$l(ref)-1),"(",2,999999)

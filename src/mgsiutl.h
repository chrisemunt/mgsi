...
typedef unsigned long UNS_32_BITS;
...
int                  mg_mutex_create            (MUTOBJ ** p_mutex_id, char * mutex_name);
int                  mg_mutex_lock              (MUTOBJ *p_mutex_id);
int                  mg_mutex_unlock            (MUTOBJ *p_mutex_id);
int                  mg_mutex_destroy           (MUTOBJ *p_mutex_id);

int                  mg_semaphore_create        (SEMOBJ ** p_sem_id, char *sem_name);
int                  mg_semaphore_lock          (SEMOBJ *p_sem_id);
int                  mg_semaphore_unlock        (SEMOBJ *p_sem_id);
int                  mg_semaphore_destroy       (SEMOBJ *p_sem_id);

unsigned long        mg_shm_size                (void);
...
int                  mg_thread_create           (LPTHRCTRL p_thread_control, MG_THR_START_ROUTINE, void *arg);
...
int                  mg_b64_strp_encoded_buf    (char *buf, int length);
